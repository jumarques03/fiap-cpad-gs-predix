...
import { View, Text, StyleSheet, ImageBackground, Platform, ScrollView, KeyboardAvoidingView, Animated, ActivityIndicator } from "react-native";
...
export default function Energia() {
  ...
  // 1. CONDICIONAL SÍNCRONA DE ALERTA IMEDIATO (Roda a cada alteração de dados)
  useEffect(() => {
    const combustivelAtual = telemetriaAtual.combustivel;
    const limiteMinCombustivel = Number(limites.minCombustivel);

    if (limiteMinCombustivel && combustivelAtual < limiteMinCombustivel) {
      if (!alertaAtivoRef.current) {
        mostrarPopUp(`⚠️ CRÍTICO: Nível de combustível (${combustivelAtual}%) abaixo do mínimo (${limiteMinCombustivel}%)!`);
      }
    } else {
      if (alertaAtivoRef.current) esconderPopUp();
    }
  }, [telemetriaAtual.combustivel, limites.minCombustivel]);

  // 2. CHAMADA ASSÍNCRONA DA IA (Roda apenas uma vez na montagem da tela)
  useEffect(() => {
    async function obterAnaliseEnergia() {
      ...
      try {
        ...
        setTextoIA("⚠️ ANOMALIA: Falha ao carregar diagnóstico do sistema de propulsão.");
      } finally {
      ...
      <View style={styles.overlay}>
        ...
        <SafeAreaView style={styles.headerContainer}>
          <View style={styles.headerRow}>
            <View>
              <Text style={styles.tituloEcra}>Combustível</Text>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            <FuelCard porcentagem={telemetriaAtual.combustivel} capacidadeTotal={"50.000 L"} />
            <DailyConsumptionCard dados={telemetriaPassada.consumo_semanal_combustivel} />
            
            {carregandoIA ? (
              <View style={styles.loadingContainer}>
                <ActivityIndicator size="small" color="#4e21ca" />
                <Text style={styles.loadingTexto}>IA interpretando matriz energética...</Text>
              </View>
            ) : (
