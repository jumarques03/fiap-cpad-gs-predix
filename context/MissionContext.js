...
export function MissionProvider({ children }) {
    ...
    // --- OBJETO ESTRUTURADO DE TELEMETRIA EXPANDIDO ---
    ...
    const [telemetriaPassada, setTelemetriaPassada] = useState({
        ...
        // Amostragem robusta de consumo (Últimos 7 dias)
        consumo_semanal_combustivel: [42, 55, 68, 55, 30, 60, 80],
        // Histórico de estabilidade expandido para contextualizar o gráfico
        historico_estabilidade_orbital_km_min: [458, 462, 460, 464, 461, 459, 463, 462, 460, 461],
        // Histórico detalhado de variações térmicas nas últimas 24 horas
        historico_temperatura_24h: [-15.0, -14.2, -13.5, -13.0, -12.5, -12.0, -11.8, -12.2, -12.8, -13.1]
    });

    // MASSA DE DADOS EXPANDIDA: Filas circulares com maior amostragem de variações (20 leituras por variável)
    const filasDinamicas = useRef({
            // Simula consumo normal seguido de duas quedas drásticas de vazamento/anomalia (9.5% e 4.2%)
            combustivel: [80, 50, 24.2, 23.5, 23.0, 22.1, 77, 20.8, 19.2, 18.0, 9.5, 8.2, 7.5, 6.1, 99, 443.8, 3.8, 15.0, 20.5, 22.0],
            
            // Oscilações térmicas bruscas com picos extremos de calor (75.2°C) e frio congelante (-45.0°C)
            temperatura: [-12.0, -5.5, 14.2, 32.8, 365, 42.1, 177.6, -8.9, -24.3, -45.0, -30.2, -12.1, 3.8, 18.9, -100, 11.0, -5.2, -18.7, -22.1, -15.8],
            
            // Descompressões rápidas e picos de pressurização (quedas até 450.0 hPa e picos de 1250.0 hPa)
            ...
            // Inserção de tempestades solares simuladas com picos violentos de radiação (8.5 mSv e 12.4 mSv)
            radiacao: [0.2, 0.25, 0.52, 1.8, 4.5, 8.5, 12.4, 6.2, 3.1, 1.4, 0.5, 0.2, 0.18, 0.45, 2.3, 7.8, 10.1, 4.9, 1.2, 0.3],
            
            // Secura extrema induzida e picos de saturação de umidade (quedas até 12.0% e picos de 95.0%)
            umidade: [42.0, 38.5, 29.1, 18.4, 12.0, 15.2, 28.0, 40.5, 55.2, 78.9, 95.0, 82.3, 64.1, 50.0, 44.7, 41.2, 42.8, 45.1, 43.5, 42.2],
            
            // Simulação de perda de sustentação orbital ou detritos espaciais com desvios graves (18.4 km e 26.9 km)
    ...
    // Motor de simulação temporal em tempo real
    useEffect(() => {
